...
class _S6SerdesClockingSimple(Module):
...
class _S6SerdesClocking(Module):
...
@ResetInserter()
class S6Serdes(Module):
    def __init__(self, pads, mode="master", data_width=4, simple_clk=True):
        if simple_clk:
            self.submodules.clocking = _S6SerdesClockingSimple(pads, mode)
        else:
            self.submodules.clocking = _S6SerdesClocking(pads, mode, data_width)
        self.submodules.tx       = _S6SerdesTX(pads, data_width)
        self.submodules.rx       = _S6SerdesRX(pads, data_width)
